...
public interface StatsRepository extends JpaRepository<EndpointHit, Long> {
    @Query("SELECT new ru.practicum.mainservice.stats.model.ViewStats(hit.app, hit.uri, COUNT(hit.uri)) " +
    ...
    @Query("SELECT new ru.practicum.mainservice.stats.model.ViewStats(hit.app, hit.uri, COUNT(hit.ip)) " +
            "FROM EndpointHit hit " +
            "WHERE hit.timestamp BETWEEN :start AND :end AND hit.uri IN :uris " +
